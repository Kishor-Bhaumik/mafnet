...
from .net import theta1,theta2,CustomNet,Decoder,Deconv,theta
...
class MemNet(nn.Module):
    def __init__(self, cfg, inplanes):
        ...
        if self.cfg.with_srm == True:
            self.aspp = build_aspp(inplanes = inplanes*2, outplanes = self.cfg.aspp_outplane)
        else:
...
#                     query_update[i] = torch.sum(query[idx].squeeze(1), dim=0)
    ...
    def read(self, query, updated_memory):
        ...
        #att1= query.contiguous().view(batch_size*h*w, dims)
